...
export function getBuyerCountryFromFundingSource(
...
export function buildContent(options: ContentConfig): string {
  ...
  const buyerCountry =
    ...
    getBuyerCountryFromFundingSource(options.fundingSource);
  const legalLocale =
    options.legalLocale ||
  ...
  if (options.errorCode) {
    content =
      LegalConstants[options.fundingSource]?.ERROR_MESSAGES?.[
        options.errorCode
      ]?.[options.legalLocale] ||
      LegalConstants[options.fundingSource]?.ERROR_MESSAGES?.[
        options.errorCode
    ...
    content =
      LegalConstants[options.fundingSource]?.LEGAL_TEXT?.[
        options.legalLocale
      ]?.(paypalPolicyLink) ||
      LegalConstants[options.fundingSource]?.LEGAL_TEXT?.[
        LegalConstants[options.fundingSource]?.DEFAULT_LOCALE
